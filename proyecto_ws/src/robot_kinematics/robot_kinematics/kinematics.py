...
class Robot():
  def __init__(self, 
    ...
    # Vector de postura
    xi_0_p = Matrix([T_0_p[0, 3],  # X
                     T_0_p[1, 3],  # Y
                     T_0_p[2, 3]]) # Z
                     
    # Jacobiano (Directo, SIN invertir)
    ...
    self.xi_0_p = xi_0_p
    self.J = J # Guardamos el Jacobiano directo
  ...
  def def_tray(self, t_f:float=2, frec:float=15, 
    ...
    # Arreglos para almacenar muestreo de Tiempo y E.F.
    ...
    # Agregar valor conocido inicial
    th_m[:, 0] = Matrix([th_i[0], th_i[1], th_i[2]])
    
    # Muestreo de las juntas
    for i in range(self.muestras):
      
      # 1. Convertimos la Jacobiana a números exactos en esta postura
      J_num = self.J.subs({
        self.th1: th_m[0, i],
        self.th2: th_m[1, i],
        self.th3: th_m[2, i]
      ...
      # 2. Obtenemos las velocidades deseadas en este punto (X, Y, Z)
      v_des = Matrix([xi_dot_m[0, i],
                      xi_dot_m[1, i],
                      xi_dot_m[2, i]])
                      
      # 3. Calculamos la inversa numéricamente para obtener velocidades de las juntas
      try:
          th_dot_m[:, i] = J_num.inv() * v_des
      except ValueError:
          print("¡Singularidad geométrica alcanzada! El robot no puede estirarse más.")
          break
  ...
  def imp_tray(self):
    ...
    z_g.plot(self.t_m.T,  self.xi_m[2, :].T, color="blue")
    plt.show()
  ...
  def imp_junt(self):
    ...
    th3_g.plot(self.t_m.T,  self.th_m[2, :].T, color="blue")
    plt.show()
